...
if (
    ...
    if ($db->count > 0) {
        ...
        if ($user_det['status'] === '1') {
            if($user_det['two_step'] === 'on'){
                $active_token = uniqid(mt_rand(10,99));
                $update_array = array(
                    "activation_code" => $active_token
                );
                $db->where("id", $user_det['id']);
                if ($db->update("members", $update_array)) {
                    activity_log($user_det['id'], $_SERVER['REQUEST_URI'],2, "members", "ارسال کد تایید برای ورود دو مرحله ای");

                    $to = $user_det['email'];
                    $subject = 'کد ورود دو مرحله ای | ایران دوج';
  ...
  <!-- end global email wrapper -->
                    ...
                    if(mail($to, $subject, $message, $headers)){
                        $REST->responseArray = array(
                            ...
                            "two_step" => 'on',
                            "type" => 'warning',
                            "message" => "ایمیل تائید هویت برای شما ارسال شده است. لطفا صبر کنید...",
                            "data" => array(
                        ...
                        activity_log(0, $_SERVER['REQUEST_URI'], 2, "members", "خطای مهم سرور در ورود به حساب کاربری");
                    }
                    ...
                    activity_log(0, $_SERVER['REQUEST_URI'], 2, "members", "خطای سرور در ورود به حساب کاربری");
                }


            }else if ($user_det['two_step'] === 'off'){
                ...
                $db->where("id", $user_det['id']);
                if ($db->update("members", $update_array)) {
                    activity_log($user_det['id'], $_SERVER['REQUEST_URI'], 2, "members", "ورود به حساب کاربری");
                    ...
                    activity_log(0, $_SERVER['REQUEST_URI'],1, "members", "ورود ناموفق به پنل کاربری");
                }
            }
        } else {
            $REST->responseArray = array(
    ...
    activity_log(0, $_SERVER['REQUEST_URI'],1, "login_user", "ورود ناموفق به پنل کاربری");
}
