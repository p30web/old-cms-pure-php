...
if (
    ...
    $db->where("id", $pid);
    $db->where("status", "1");
    $fetch_plan = $db->getOne("dollar_investment_plan");
    ...
    if(
        ...
        if($fetch_mem['dollar_credit'] >= $fetch_plan['price']){
            $tnow = time();
            ...
            if($db->insert("active_investment_plan", $insert_active_plan)){
                ...
                $update_member = array(
                    "dollar_credit"=> $fetch_mem['dollar_credit'] - $fetch_plan['price']
                );
                $db->where("id", $uid);
                if($db->update("members", $update_member)){
                    activity_log($uid, $_SERVER['REQUEST_URI'],1, "active_investment_plan", "خرید پلن دلاری جدید به مبلغ ". $fetch_plan['price']);
