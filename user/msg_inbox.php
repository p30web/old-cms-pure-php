...
<head>
    <title>ایران دوج | پیام های من</title>
                                    ...
                                    <div class="col-xl-12 col-lg-12 order-lg-12 order-xl-12">
                                        <div class="kt-portlet kt-portlet--height-fluid">
                                            ...
                                            <div class="kt-portlet__body">
                                                ...
                                                if(in_array($_SESSION['member_id'],array(98,24)) && !is_null($fetch_admin) && count($fetch_admin[0]) != 0){
                                                    $query_member_messages = sprintf("SELECT * FROM `member_messages` WHERE `member_id` = %d ORDER BY `created_at` DESC", $fetch_admin[0]['admin_id']);


                                                }else{
                                                    $query_member_messages = sprintf("SELECT * FROM `member_messages` WHERE `member_id` = %d ORDER BY `created_at` DESC", $_SESSION['member_id']);
                                                }
                                                
                                                $member_messages = mysqli_query($cn, $query_member_messages) or die(mysqli_error($cn));
                                                $row_member_msg = mysqli_fetch_assoc($member_messages);
                                                $totalRows_member_messages = mysqli_num_rows($member_messages);
                                                if($totalRows_member_messages > 0) {
                                                    ?>
                                                    <div class="kt-notification kt-notification--fit">
                                                        <?php
                                                        do {
                                                            ?>
                                                            <a href="#" onclick="MsgDetails(<?php echo $row_member_msg['id']; ?>)" class="kt-notification__item">
                                                                <div class="kt-notification__item-icon">
                                                                    <i class="flaticon2-mail-1 <?php if($row_member_msg['seen'] == 0) echo 'kt-font-success'; else echo 'kt-font-dark'; ?>"></i>
                                                                </div>
                                                                <div class="kt-notification__item-details">
                                                                    <div class="kt-notification__item-title">
                                                                       <?php echo $row_member_msg['title']; ?>
                                                                    </div>
                                                                    <div class="kt-notification__item-time">
                                                                        <?php echo ago_time(jmktime( substr($row_member_msg['created_at'], 11,2) , substr($row_member_msg['created_at'], 14,2) , substr($row_member_msg['created_at'], 17,2) , substr($row_member_msg['created_at'], 5,2) , substr($row_member_msg['created_at'], 8,2) , substr($row_member_msg['created_at'], 0,4) , "", 'Asia/Tehran')); ?>
                                                                    </div>
                                                                </div>
                                                            </a>
                                                            <?php
                                                        } while ($row_member_msg = mysqli_fetch_assoc($member_messages));
                                                        ?>
                                                    </div>
                                                    <?php
                                                }else{
                                                    echo '<div class="kt-grid kt-grid--hor kt-grid__item kt-grid__item--fluid kt-grid__item--middle">
                                                                    <div class="kt-grid__item kt-grid__item--middle kt-align-center">
                                                                        کاربر گرامی!
                                                                        <br>پیامی برای شما ارسال نشده است!
                                                                    </div>
                                                                </div>';
                                                }
                                                ?>
                                        ...
                                        <!--begin::Modal-->
                                        <div class="modal fade" id="msgdetails" tabindex="-1" role="dialog" aria-labelledby="نمایش جزئیات تراکنش" aria-hidden="true">
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="msgtitle">نمایش جزئیات تراکنش</h5>
...
<script>
    ...
    function MsgDetails(i) {
        $.ajax({
            url: 'api/msg_details.php',
            data: {id: i},
            method: "GET",
            success: function (data) {
                $('#msgdetails').modal('show');
                $('#msgtext').html(data.data.text);
