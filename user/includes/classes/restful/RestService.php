...
use Exception;
use lib\classes\Authentication;
class RestService
{
    ...
    public function RseponseToC()
    {
        try {
            ...
            elseif ($this->Authenticated === false)
                ...
                {
                   $res = json_encode($this->responseArray, JSON_UNESCAPED_UNICODE);
                   $crypto = new \lib\classes\Crypto();
                   $crypto->r = $res;
                   $crypto->n = time();
                ...
                elseif (!in_array('GET', $this->method) && $this->responseType ==  \lib\classes\RestFormat::PLAIN){
                    header('HTTP/1.0 '.$this->responseArray['status'].' '.$this->responseArray['msg']);
