...
<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    ...
    <title>Rahaaa | مدیریت مطالب</title>
				...
				<i class="kt-font-brand flaticon2-back"></i>
                            ...
                            <div class="kt-portlet__head-toolbar">
                                <div class="kt-portlet__head-wrapper">
                                    <div class="kt-portlet__head-actions">
                                        <?php
                                        if (in_array("post_add", $privilegeslist)) echo '<a href="post_add.php" class="btn btn-brand btn-elevate btn-icon-sm"><i class="la la-plus"></i>رکورد جدید</a>';
                                        ?>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="kt-portlet__body">
                            ...
                            <table class="table table-striped- table-bordered table-hover table-checkable"
                                ...
                                $query_posts = sprintf("SELECT * FROM `posts` ORDER BY id ASC");
                                $posts = mysqli_query($cn, $query_posts) or die(mysqli_error($cn));
                                $row_posts= mysqli_fetch_assoc($posts);
                                echo $totalRows_posts = mysqli_num_rows($posts);
                                    ...
                                    do {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                if($row_post_category['level'] == 0){
                                                    echo 'گروه اصلی';
                                                }else{
                                                    mysqli_select_db($cn, $database_cn);
                                                    $post_categorylvel = sprintf("SELECT `id`,`name`,`code` FROM `post_category` WHERE code= %d", $row_posts['category']);
                                                    $post_categorylvel = mysqli_query($cn, $post_categorylvel) or die(mysqli_error($cn));
                                                    $row_post_categorylvel = mysqli_fetch_assoc($post_categorylvel);
                                                    $totalRows_post_categorylvel = mysqli_num_rows($post_categorylvel);

                                                    echo $row_post_categorylvel['name'];
                                                }
                                                ?>
                                            </td>
                                            <td><?php
                                                mysqli_select_db($cn, $database_cn);
                                                $query_rssettings = sprintf("SELECT `id`,`name` FROM `settings` WHERE id= %d ", $row_posts['portalid']);
                                                ...
                                                ?></td>
                                            <td><?php echo $row_posts['sort']; ?></td>
                                            <td>
                                                <span class="kt-badge kt-badge--primary kt-badge--inline"><?php echo $row_posts['created_at']; ?></span>
                                                <span class="kt-badge kt-badge--success kt-badge--inline"><?php echo $row_posts['modified_at']; ?></span>
                                            </td>
                                            <td><?php
                                                if ($row_posts['status'] == '1') {
                                                    echo '<span class="kt-badge kt-badge--success kt-badge--inline kt-badge--pill">فعال</span>';
                                                } elseif ($row_posts['status'] == '0') {
                                            ...
                                            <td><?php
                                                if ($row_posts['view'] == '1') {
                                                    echo '<span class="kt-badge kt-badge--success kt-badge--inline kt-badge--pill">نمایش</span>';
                                                } elseif ($row_posts['view'] == '0') {
                                                    echo '<span class="kt-badge kt-badge--danger kt-badge--inline kt-badge--pill">عدم نمایش</span>';
                                                } else {
                                                    echo '<span class="kt-badge kt-badge--info kt-badge--inline kt-badge--pill">نا مشخص</span>';
                                                }
                                                ?></td>
                                                ...
                                                <span class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <?php if (in_array("change_status", $privilegeslist)) { ?><a
                                                            class="dropdown-item"
                                                            href="<?php ?>../../change_status.php?table=<?php echo encrypt('posts', session_id() . "sts"); ?>&field=<?php echo encrypt('status', session_id() . "sts"); ?>&id=<?php echo encrypt($row_posts['id'], session_id() . "sts"); ?>">
                                                        ...
                                                        <?php if (in_array("post_delete", $privilegeslist)) { ?><a
                                                            ...
                                                            href="post_delete.php?id=<?php echo encrypt($row_posts['id'], session_id() . "pst"); ?>">
                                                                <i class="la la-trash text-danger "></i> حذف
                                                            </a><?php } ?>

                                                    </div>
                                                </span>
                                                <?php if (in_array("post_edit", $privilegeslist)) { ?>
                                                    <a href="post_edit.php?id=<?php echo encrypt($row_posts['id'], session_id() . "pst"); ?>"
