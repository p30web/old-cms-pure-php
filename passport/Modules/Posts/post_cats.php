...
<head>
    ...
    <title>Rahaaa | مدیریت گروه مطالب</title>
                            ...
                            <div class="kt-portlet__head-toolbar">
                                <div class="kt-portlet__head-wrapper">
                                    <div class="kt-portlet__head-actions">
                                        <?php
                                        if (in_array("post_cat_add", $privilegeslist)) echo '<a href="post_cat_add.php" class="btn btn-brand btn-elevate btn-icon-sm"><i class="la la-plus"></i>رکورد جدید</a>';
                                        ?>
                                    </div>
                                </div>
                            </div>
                            ...
                            <table class="table table-striped- table-bordered table-hover table-checkable"
                                ...
                                if ($totalRows_post_category > 0) {
                                    do {
                                        ...
                                        <tr>
                                            <td><?php echo $row_post_category['id']; ?></td>
                                            <td><?php echo $row_post_category['name']; ?></td>
                                            <td><?php echo $row_post_category['code']; ?></td>
                                            <td>
                                                <?php
                                                ...
                                                if($row_post_category['level'] == 0){
                                                    ...
                                                    mysqli_select_db($cn, $database_cn);
                                                    $post_categorylvel = sprintf("SELECT `id`,`name`,`code` FROM `post_category` WHERE id= %d", $row_post_category['level']);
                                            ...
                                            <td><?php
                                                mysqli_select_db($cn, $database_cn);
                                                $query_rssettings = sprintf("SELECT `id`,`name` FROM `settings` WHERE id= %d ", $row_post_category['portalid']);
                                                ...
                                                <span class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <?php if (in_array("change_status", $privilegeslist)) { ?><a
                                                            class="dropdown-item"
                                                            href="<?php ?>../../change_status.php?table=<?php echo encrypt('post_category', session_id() . "sts"); ?>&field=<?php echo encrypt('status', session_id() . "sts"); ?>&id=<?php echo encrypt($row_post_category['id'], session_id() . "sts"); ?>">
                                                                <i class="la la-toggle-on"></i> تغییر وضعیت
                                                            </a><?php } ?>
                                                        <?php if (in_array("post_cat_delete", $privilegeslist)) { ?><a
                                                            class="dropdown-item text-danger"
                                                            onClick="return confirm('آیا مطمئنید می خواهید اطلاعات را حذف کنید ؟');"
                                                            href="post_cat_delete.php?id=<?php echo encrypt($row_post_category['id'], session_id() . "pct"); ?>">
                                                                <i class="la la-trash text-danger "></i> حذف
                                                            </a><?php } ?>

                                                    </div>
                                                </span>
                                                <?php if (in_array("post_cat_edit", $privilegeslist)) { ?>
                                                    <a href="post_cat_edit.php?id=<?php echo encrypt($row_post_category['id'], session_id() . "pct"); ?>"
...
<script type="text/javascript">
    ...
    var KTDatatablesBasicPaginations = {
        init: function () {
            $("#_post_category_table").DataTable({
                responsive: !0,
