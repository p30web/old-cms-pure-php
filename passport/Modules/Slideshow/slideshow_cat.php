...
<head>
    ...
    <title>Rahaaa | مدیریت اسلایدر ها</title>
				...
				<i class="kt-font-brand flaticon2-back"></i>
                            ...
                            <div class="kt-portlet__head-toolbar">
                                <div class="kt-portlet__head-wrapper">
                                    <div class="kt-portlet__head-actions">
                                        <?php
                                        if (in_array("slideshow_cat_add", $privilegeslist)) echo '<a href="slideshow_cat_add.php" class="btn btn-brand btn-elevate btn-icon-sm"><i class="la la-plus"></i>رکورد جدید</a>';
                                        ?>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="kt-portlet__body">
                            ...
                            <table class="table table-striped- table-bordered table-hover table-checkable"
                                ...
                                $query_slideshowcat= sprintf("SELECT * FROM `slideshowcat` ORDER BY id ASC");
                                ...
                                if ($totalRows_slideshowcat > 0) {
                                    do {
                                        ...
                                        <tr>
                                            <td><?php echo $row_slideshowcat['id']; ?></td>
                                            <td><?php echo $row_slideshowcat['code']; ?></td>
                                            <td><?php echo $row_slideshowcat['name']; ?></td>
                                            <td><?php echo $row_slideshowcat['sort']; ?></td>
                                            <td><?php
                                                if ($row_slideshowcat['status'] == '1') {
                                                    echo '<span class="kt-badge kt-badge--success kt-badge--inline kt-badge--pill">فعال</span>';
                                                } elseif ($row_slideshowcat['status'] == '0') {
                                                ...
                                                <span class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <?php if (in_array("change_status", $privilegeslist)) { ?><a
                                                            class="dropdown-item"
                                                            href="<?php ?>../../change_status.php?table=<?php echo encrypt('slideshowcat', session_id() . "sts"); ?>&field=<?php echo encrypt('status', session_id() . "sts"); ?>&id=<?php echo encrypt($row_slideshowcat['id'], session_id() . "sts"); ?>">
                                                                <i class="la la-toggle-on"></i> تغییر وضعیت
                                                            </a><?php } ?>
                                                        <?php if (in_array("slideshow_cat_delete", $privilegeslist)) { ?><a
                                                            class="dropdown-item text-danger"
                                                            onClick="return confirm('آیا مطمئنید می خواهید اطلاعات را حذف کنید ؟');"
                                                            href="slideshow_cat_delete.php?id=<?php echo encrypt($row_slideshowcat['id'], session_id() . "slc"); ?>">
                                                                <i class="la la-trash text-danger "></i> حذف
                                                            </a><?php } ?>

                                                    </div>
                                                </span>
                                                <?php if (in_array("slideshow_cat_edit", $privilegeslist)) { ?>
                                                    <a href="slideshow_cat_edit.php?id=<?php echo encrypt($row_slideshowcat['id'], session_id() . "slc"); ?>"
