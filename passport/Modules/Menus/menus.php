				...
				<i class="kt-font-brand flaticon2-back"></i>
                            ...
                            <div class="kt-portlet__head-toolbar">
                                <div class="kt-portlet__head-wrapper">
                                    <div class="kt-portlet__head-actions">
                                        <?php
                                        if (in_array("menu_add", $privilegeslist)) echo '<a href="menu_add.php" class="btn btn-brand btn-elevate btn-icon-sm"><i class="la la-plus"></i>رکورد جدید</a>';
                                        ?>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="kt-portlet__body">
                            ...
                            <table class="table table-striped- table-bordered table-hover table-checkable"
                                ...
                                $query_rsmenus= sprintf("SELECT * FROM `menus` ORDER BY id ASC");
                                ...
                                if ($totalRows_rsmenus > 0) {
                                    do {
                                        ...
                                        <tr>
                                            <td><?php echo $row_rsmenus['id']; ?></td>
                                            <td><?php echo $row_rsmenus['title']; ?></td>
                                            <td><?php echo $row_rsmenus['url']; ?></td>
                                            <td><?php
                                                if($row_rsmenus['type'] === '0'){
                                                    echo '<span class="kt-badge kt-badge--danger kt-badge--inline">والد</span>';
                                                }elseif ($row_rsmenus['type'] === '1'){

                                                    mysqli_select_db($cn, $database_cn);
                                                    $query_rsmenusparent= sprintf("SELECT id,title FROM `menus` WHERE id= ".$row_rsmenus['parent_id']." ");
                                                    $rsmenusparent = mysqli_query($cn, $query_rsmenusparent) or die(mysqli_error($cn));
                                                    $row_rsmenusparent = mysqli_fetch_assoc($rsmenusparent);
                                                    $totalRows_rsmenusparent = mysqli_num_rows($rsmenusparent);

                                                    echo '<span class="kt-badge kt-badge--warning kt-badge--inline">فرزند | '.$row_rsmenusparent['title'].'</span>';
                                                }
                                                ?></td>
                                            <td><?php echo $row_rsmenus['sort']; ?></td>

                                            <td><?php
                                                if ($row_rsmenus['status'] == '1') {
                                                    echo '<span class="kt-badge kt-badge--success kt-badge--inline kt-badge--pill">فعال</span>';
                                                } elseif ($row_rsmenus['status'] == '0') {
                                                ...
                                                <span class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <?php if (in_array("change_status", $privilegeslist)) { ?><a
                                                            class="dropdown-item"
                                                            href="<?php ?>../../change_status.php?table=<?php echo encrypt('menus', session_id() . "sts"); ?>&field=<?php echo encrypt('status', session_id() . "sts"); ?>&id=<?php echo encrypt($row_rsmenus['id'], session_id() . "sts"); ?>">
                                                                <i class="la la-toggle-on"></i> تغییر وضعیت
                                                            </a><?php } ?>
                                                        <?php if (in_array("menu_delete", $privilegeslist)) { ?><a
                                                            class="dropdown-item text-danger"
                                                            onClick="return confirm('آیا مطمئنید می خواهید اطلاعات را حذف کنید ؟');"
                                                            href="menu_delete.php?id=<?php echo encrypt($row_rsmenus['id'], session_id() . "mnu"); ?>">
                                                                <i class="la la-trash text-danger "></i> حذف
                                                            </a><?php } ?>

                                                    </div>
                                                </span>
                                                <?php if (in_array("menu_edit", $privilegeslist)) { ?>
                                                    <a href="menu_edit.php?id=<?php echo encrypt($row_rsmenus['id'], session_id() . "mnu"); ?>"
