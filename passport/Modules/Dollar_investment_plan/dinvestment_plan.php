...
<head>
    ...
    <title>Rahaaa | پلن های سرمایه گذاری دلاری</title>
				...
				<i class="kt-font-brand flaticon2-back"></i>
                            ...
                            <div class="kt-portlet__head-toolbar">
                                <div class="kt-portlet__head-wrapper">
                                    <div class="kt-portlet__head-actions">
                                        <?php
                                        if (in_array("dinvestment_plan_add", $privilegeslist)) echo '<a href="dinvestment_plan_add.php" class="btn btn-brand btn-elevate btn-icon-sm"><i class="la la-plus"></i>رکورد جدید</a>';
                                        ?>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="kt-portlet__body">
                            ...
                            <table class="table table-striped- table-bordered table-hover table-checkable"
                                ...
                                $query_investment_plan = sprintf("SELECT * FROM `dollar_investment_plan` ORDER BY id ASC");
                                    ...
                                    do {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php
                                                mysqli_select_db($cn, $database_cn);
                                                $query_rsunits = sprintf("SELECT * FROM `unit` WHERE id= %d", $row_investment_plan['unit_id']);
                                                ...
                                                <span class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <?php if (in_array("change_status", $privilegeslist)) { ?><a
                                                            class="dropdown-item"
                                                            href="<?php ?>../../change_status.php?table=<?php echo encrypt('dollar_investment_plan', session_id() . "sts"); ?>&field=<?php echo encrypt('status', session_id() . "sts"); ?>&id=<?php echo encrypt($row_investment_plan['id'], session_id() . "sts"); ?>">
                                                                <i class="la la-toggle-on"></i> تغییر وضعیت
                                                            </a><?php } ?>
                                                        <?php if (in_array("investment_plan_delete", $privilegeslist)) { ?><a
                                                            class="dropdown-item text-danger"
                                                            onClick="return confirm('آیا مطمئنید می خواهید اطلاعات را حذف کنید ؟');"
                                                            href="dinvestment_plan_delete.php?id=<?php echo encrypt($row_investment_plan['id'], session_id() . "inv"); ?>">
                                                                <i class="la la-trash text-danger "></i> حذف
                                                            </a><?php } ?>

                                                    </div>
                                                </span>
                                                <?php if (in_array("investment_plan_edit", $privilegeslist)) { ?>
                                                    <a href="dinvestment_plan_edit.php?id=<?php echo encrypt($row_investment_plan['id'], session_id() . "inv"); ?>"
